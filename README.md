...
- `POST /mix` — JSON `{ "job_id": "<이전 /separate 응답의 job_id>", "mixes": [{ "key": "...", "stems": [...] }] }`,
  ...
  - `key`는 요청한 값 그대로 응답에 돌아옴(백엔드는 의미를 해석하지 않고 결과 매핑용으로만 씀) — 같은 요청
    안에서 `key`가 중복되면 400.
  - `stems`가 1개뿐이고 볼륨 조절도 없으면 믹싱 없이 원본 stem의 공개 URL을 그대로 돌려줌(비용 0).
  - `volumes`(선택): `{ "vocals": 1.5, "other": 0.4 }`처럼 stem별 볼륨 배율을 지정할 수 있음. `0.0`(무음)~`2.0`(200%)
    범위, 생략된 stem은 `1.0`(원곡 그대로). `stems`에 없는 stem을 `volumes`에 넣거나 범위를 벗어나면 400.
    ffmpeg `amix`가 `normalize=0`(자동 볼륨 축소 없음)라 여러 파트를 동시에 키우면 소리가 찌그러질 수 있음 —
    서버가 자동으로 막지 않으므로(투명하고 예측 가능한 동작을 우선함) 프론트에서 안내 필요.
